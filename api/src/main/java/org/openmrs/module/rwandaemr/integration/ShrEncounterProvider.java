...
public class ShrEncounterProvider {
    ...
    /**
     * Dedicated timeout (in milliseconds) for SHR encounter fetch operation.
     * Can be overridden with global property: rwandaemr.hie.shrEncounterFetchTimeoutMs
     */
    private static final String SHR_FETCH_TIMEOUT_MS_GP = "rwandaemr.hie.shrEncounterFetchTimeoutMs";
    private static final int DEFAULT_SHR_FETCH_TIMEOUT_MS = 20000;
    ...
    public List<ShrEncounter> fetchEncounterFromShr(String upid){
        ...
        try(CloseableHttpClient httpClient = HttpUtils.getHieClient()){
            ...
            httpGet.setConfig(RequestConfig.copy(RequestConfig.DEFAULT)
                    ...
                    .build());
            log.info("Getting Encounters for " + upid + " from " + url);
                ...
                if(data != null && !data.trim().isEmpty()){
                    try{
                        ...
                        if(resource instanceof OperationOutcome){
                            ...
                            log.info("Full OperationOutcome response: " + data);
                        ...
                        log.info("Failed to parse response as OperationOutcome: " + parseException.getMessage());
                    }
                }
            ...
            log.info("Re-throwing IllegalStateException: " + ise.getMessage());
            throw ise;
        } catch (Exception e) {
            log.info("Unable to get information from SHR registry: " + e.getMessage(), e);
        }
        log.info("Returning null information");
        return null;
    }

    private int resolveShrFetchTimeoutMs() {
        try {
            String configured = ConfigUtil.getProperty(SHR_FETCH_TIMEOUT_MS_GP);
            if (configured != null && configured.trim().length() > 0) {
                int parsed = Integer.parseInt(configured.trim());
                if (parsed > 0) {
                    return parsed;
                }
                log.warn("Ignoring non-positive " + SHR_FETCH_TIMEOUT_MS_GP + " value: " + configured);
            }
        } catch (Exception e) {
            log.warn("Invalid " + SHR_FETCH_TIMEOUT_MS_GP + " value, using default " + DEFAULT_SHR_FETCH_TIMEOUT_MS + " ms");
        }
        return DEFAULT_SHR_FETCH_TIMEOUT_MS;
    }
